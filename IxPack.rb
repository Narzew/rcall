...
module IxPack
	...
	def self.decrypt_data(filename, arch, key="")
		if key != nil && key != ""
			$master_key2 = key
		end
	...
	def self.decrypt_file(filename, arch, key,out="")
		if out == nil || out == ""
			out = filename
		end
		File.open(out,'wb'){|f|w.write(IxPack.decrypt_stream(filename,arch,key)}
	end
end
		
		
begin
	print "IxPack v 1.1 by Narzew\n"
	if ARGV.size == 0
		...
		if ARGV[4] != nil && ARGV[4] != ""
			IxPack.decrypt_file(ARGV[1],ARGV[2],ARGV[3],ARGV[4])
		else
